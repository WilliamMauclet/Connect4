...
class MinmaxRobot_AlphaBeta(FirstOrderRobot):
    ...
    def apply_leaf_heuristic_alphabeta(self, grid) -> int:
        return self.apply_leaf_heuristic(grid, grid.get_last_move()[0])

    def alphabeta(self, grid, depth: int, alpha: int, beta: int, max_not_min: bool):
        if depth == 0:
            return self.apply_leaf_heuristic_alphabeta(grid)
        ...
        if max_not_min:
            ...
            for x in grid.get_free_columns():
                ...
                v = max(v, self.alphabeta(new_grid, depth - 1, alpha, beta, False))
                alpha = max(alpha, v)
                if beta <= alpha:
                    break
            return v
            ...
            for x in grid.get_free_columns():
                ...
                v = min(v, self.alphabeta(new_grid, depth - 1, alpha, beta, True))
                beta = min(beta, v)
                if beta <= alpha:
                    break
            return v
    ...
    def choose_move_with_minmax(self, grid) -> int:
        ...
        for x in grid.get_free_columns():
            ...
            moves_scores.append({'move': x, 'score': self.alphabeta(new_grid, self.DEPTH - 1, -inf, +inf, False)})
